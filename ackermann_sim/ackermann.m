...
next_state = calc_increment(s_v, s_theta, s_w, s_delta) * s_dt + [s_px; s_py; s_theta, s_psi];
...
sim_time = 1; % 20 seconds simulation
...
for i = 1:length(time)
    % calculate the increment
    increment = calc_increment(10, state(3), 0, 0.1);
    % calculate next state
    state = calc_next_state(10, state(3), 0, -0.1, state(1), state(2), h_cont);
    % plot
